...
@section('content')
    ...
    <p>
        ...
        @if(!is_null(Auth::getUser()->role_id) && config('global.can_see_all_records_role_id') == Auth::getUser()->role_id)
            @if(Session::get('Provider.filter', 'all') == 'my')
            ...
            <table class="table table-bordered table-striped ajaxTable @can('provider_delete') @if ( request('show_deleted') != 1 ) dt-select @endif @endcan">
                <thead>
                    <tr>
                        ...
                        <th>@lang('global.providers.fields.network-affiliate')</th>
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            window.dtDefaultOptions.columns = [@can('provider_delete')
                ...
                @endif
                @endcan{data: 'provider', name: 'provider'},
                {data: 'network_affiliate.network_affiliate', name: 'network_affiliate.network_affiliate'},
                {data: 'network_affiliate.network', name: 'network_affiliate.network'},
