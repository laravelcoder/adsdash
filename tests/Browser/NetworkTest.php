...
class NetworkTest extends DuskTestCase
{
    ...
    public function testCreateNetwork()
    {
        ...
        $this->browse(function (Browser $browser) use ($admin, $network, $relations) {
            $browser->loginAs($admin)
                ...
                ->select('select[name="provider[]"]', $relations[0]->id)
                ->select('select[name="provider[]"]', $relations[1]->id)
                ->press('Save')
                ...
                ->assertSeeIn("tr:last-child td[field-key='provider'] span:first-child", $relations[0]->provider)
                ->assertSeeIn("tr:last-child td[field-key='provider'] span:last-child", $relations[1]->provider);
        });
    }
    ...
    public function testShowNetwork()
    {
        ...
        $network->provider()->attach([$relations[0]->id, $relations[1]->id]);
