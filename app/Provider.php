<?php
namespace App;

use Illuminate\Database\Eloquent\Model;
use Illuminate\Database\Eloquent\SoftDeletes;
use App\Traits\FilterByUser;
use App\Traits\FilterByUser;

/**
 * Class Provider
 *
 * @package App
 * @property string $provider
 * @property string $created_by
 * @property string $created_by_team
 * @property string $network_affiliate
*/
class Provider extends Model
{
    use SoftDeletes, FilterByUser, FilterByUser;

    protected $fillable = ['provider', 'created_by_id', 'created_by_team_id', 'network_affiliate_id'];
    protected $hidden = [];
    public static $searchable = [
        'provider',
    ];
    

    /**
     * Set to null if empty
     * @param $input
     */
    public function setCreatedByIdAttribute($input)
    {
        $this->attributes['created_by_id'] = $input ? $input : null;
    }

    /**
     * Set to null if empty
     * @param $input
     */
    public function setCreatedByTeamIdAttribute($input)
    {
        $this->attributes['created_by_team_id'] = $input ? $input : null;
    }

    /**
     * Set to null if empty
     * @param $input
     */
    public function setNetworkAffiliateIdAttribute($input)
    {
        $this->attributes['network_affiliate_id'] = $input ? $input : null;
    }
    
    public function created_by()
    {
        return $this->belongsTo(User::class, 'created_by_id');
    }
    
    public function created_by_team()
    {
        return $this->belongsTo(Team::class, 'created_by_team_id');
    }
    
    public function network_affiliate()
    {
        return $this->belongsTo(Network::class, 'network_affiliate_id')->withTrashed();
    }
    
    public function stations() {
        return $this->hasMany(Station::class, 'provider_id');
    }
}
