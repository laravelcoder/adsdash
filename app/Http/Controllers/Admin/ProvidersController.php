...
class ProvidersController extends Controller
{
    ...
    public function show($id)
    {
        ...
        $created_bies = \App\User::get()->pluck('name', 'id')->prepend(trans('global.app_please_select'), '');
        $created_by_teams = \App\Team::get()->pluck('name', 'id')->prepend(trans('global.app_please_select'), '');
        $network_affiliates = \App\Network::get()->pluck('network_affiliate', 'id')->prepend(trans('global.app_please_select'), '');$stations = \App\Station::where('provider_id', $id)->get();

        $provider = Provider::findOrFail($id);

        return view('admin.providers.show', compact('provider', 'stations'));
    }
