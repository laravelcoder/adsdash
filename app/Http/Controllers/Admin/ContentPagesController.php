...
class ContentPagesController extends Controller
{
    ...
    public function create()
    {
        ...
        return view('admin.content_pages.create', compact('category_ids', 'tag_ids', 'templates'));
    }
    ...
    public function edit($id)
    {
        ...
        return view('admin.content_pages.edit', compact('content_page', 'category_ids', 'tag_ids', 'templates'));
    }
    ...
    public function show($id)
    {
        ...
        $templates = \App\Template::get()->pluck('template_name', 'id')->prepend(trans('global.app_please_select'), '');$templates = \App\Template::whereHas('pages',
                    function ($query) use ($id) {
                        $query->where('id', $id);
                    })->get();$stylesheets = \App\Stylesheet::whereHas('pages',
                    function ($query) use ($id) {
                        $query->where('id', $id);
                    })->get();$bottom_scripts = \App\BottomScript::whereHas('pages',
                    function ($query) use ($id) {
                        $query->where('id', $id);
                    })->get();$top_scripts = \App\TopScript::whereHas('pages',
                    function ($query) use ($id) {
                        $query->where('id', $id);
                    })->get();
        ...
        return view('admin.content_pages.show', compact('content_page', 'templates', 'stylesheets', 'bottom_scripts', 'top_scripts'));
    }
